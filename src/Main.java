Simport java.text.SimpleDateFormat;
	...
	public static void main(String[] args) {
		...
		try{
		      ...
		      for(int i=0;i<stud.length;i++)
		      {
		           ...
		           stud[i]=new Student(i+1,"Shravani"+i,date1,8.74+(double)i);
		      }
		      for(int i=0;i<stud.length;i++)
		      {
		      ...
		      String sDate1="1994-08-18";
                  Date date1=new SimpleDateFormat("yyyy-MM-dd").parse(sDate1);
		      Student p=new Student(6,"Navya",date1,8.00);
		      gp.setStudent(p,2);
		      for(int i=0;i<stud.length;i++)
		      {
		            System.out.println(stud[i].getId()+"  "+stud[i].getFullName()+"  "+stud[i].getBirthDate()+"  "+stud[i].getAvgMark());
		      ...
		      Student q=new Student(12,"srvs",date2,8.00);
